...
function stringifyParams(params) {
  if (!params) return "";
  let parts = Object.keys(params).sort().filter((k) => params[k] !== void 0).map((k) => {
...
var View = class {
  ...
  async render(renderer) {
    const { isPreview: isPreview2, shouldRender, newSnapshot: snapshot } = renderer;
    if (shouldRender) {
      try {
        ...
        this.delegate.viewRenderedSnapshot(snapshot, isPreview2);
...
var PageView = class extends View {
  ...
  renderPage(snapshot, isPreview2 = false, willRender = true, visit2) {
    const renderer = new PageRenderer(this.snapshot, snapshot, PageRenderer.renderElement, isPreview2, willRender);
...
function walk(obj) {
  ...
  return Object.keys(obj).reduce((acc, key) => {
    let camel = key[0].toLowerCase() + key.slice(1).replace(/([A-Z]+)/g, (m, x) => {
...
// presence_source_element.js
var TurboPresenceSourceElement = class extends HTMLElement {
  static cable;
  static delayedUnsubscribe;
  async connectedCallback() {
    connectStreamSource(this);
...
function start2(cable, opts = {}) {
  ...
  if (enablePresence) {
    ...
    if (customElements.get(presenceTagName) === void 0) {
